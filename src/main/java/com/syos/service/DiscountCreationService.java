...
/**
 * Handles interactive prompts to create a new discount.
 */
public class DiscountCreationService {
	...
	public int createDiscount() {
		...
		if (discountName.isEmpty()) {
			System.out.println("⚠️  Discount name cannot be empty.");
		...
		try {
			...
			System.out.println("⚠️  Invalid discount type. Use PERCENT or AMOUNT.");
			return -1;
		}
		...
		try {
			...
			if (discountValue < 0) {
				System.out.println("⚠️  Discount value must be non-negative.");
				return -1;
			}
			if (discountType == DiscountType.PERCENT && discountValue > 100) {
				System.out.println("⚠️  Percentage cannot exceed 100.");
				return -1;
			}
		} catch (NumberFormatException e) {
			System.out.println("⚠️  Invalid number format for discount value.");
			return -1;
		}
		...
		try {
			...
			System.out.println("⚠️  Invalid start date format.");
			return -1;
		}
		...
		try {
			...
			System.out.println("⚠️  Invalid end date format.");
			return -1;
		}
