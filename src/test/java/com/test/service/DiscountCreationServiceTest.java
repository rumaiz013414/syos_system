...
class DiscountCreationServiceTest {
    ...
    // --- Test Cases for Repository Interaction / Errors ---

    @Test
    @DisplayName("Should handle repository returning -1 (failure to create)")
    void createDiscount_RepoReturnsMinusOne() {
        // Arrange
        String name = "Test";
        String type = "AMOUNT";
        String value = "10.0";
        ...
        when(mockDiscountRepo.createDiscount(any(), any(), anyDouble(), any(), any())).thenReturn(-1); // Simulate failure

        // Act
        int resultId = discountCreationService.createDiscount();

        // Assert
        assertEquals(-1, resultId, "Should return -1 on repository failure");
        assertTrue(outContent.toString().contains("Error: Failed to create discount (repository returned -1)."));

        verify(mockDiscountRepo, times(1)).createDiscount(
                name, DiscountType.AMOUNT, 10.0, LocalDate.parse(startDate), LocalDate.parse(endDate)
        );
    }

    @Test
    @DisplayName("Should handle RuntimeException from repository")
    void createDiscount_RepoThrowsRuntimeException() {
        // Arrange
        String name = "Error Discount";
        String type = "PERCENT";
        String value = "5";
        String startDate = "2025-08-01";
        String endDate = "2025-08-15";
        ...
        // Simulate repository throwing a RuntimeException
        when(mockDiscountRepo.createDiscount(any(), any(), anyDouble(), any(), any()))
        ...
        // Assert
        assertEquals(-1, resultId, "Should return -1 when repository throws an exception");
        assertTrue(outContent.toString().contains("Error: An unexpected error occurred while creating the discount: Database connection lost"));

        verify(mockDiscountRepo, times(1)).createDiscount(
                name, DiscountType.PERCENT, 5.0, LocalDate.parse(startDate), LocalDate.parse(endDate)
        );
    }
}