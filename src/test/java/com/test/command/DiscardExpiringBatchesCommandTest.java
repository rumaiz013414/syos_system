...
class DiscardExpiringBatchesCommandTest {
    ...
    void shouldSuccessfullyDiscardPartialQuantity() {
        ...
        // Corrected StockBatch constructor usage: id, productCode, purchaseDate, expiryDate, quantityRemaining
        StockBatch expiringBatch = new StockBatch(batchId, "PROD001", LocalDate.now().minusMonths(1), LocalDate.now().plusDays(5), 45);
        List<StockBatch> expiringBatches = Arrays.asList(expiringBatch);

        when(scanner.nextLine())
                .thenReturn(String.valueOf(daysThreshold)) // For daysThreshold input
        ...
        assertTrue(output.contains(String.format("Batch ID: %d, Product: %s, Current Remaining Quantity: %d",
                batchId, expiringBatch.getProductCode(), expiringBatch.getQuantityRemaining())));
        assertTrue(output.contains("Enter quantity to discard from this batch"));
        assertTrue(output.contains(String.format("Successfully discarded %d units from Batch ID %d.", quantityToDiscard, batchId)));
    }
    ...
    void shouldSuccessfullyDiscardAllQuantity() {
        ...
        StockBatch expiringBatch = new StockBatch(batchId, "PROD002", LocalDate.now().minusWeeks(2), LocalDate.now().plusDays(2), remainingQuantity);
        List<StockBatch> expiringBatches = Arrays.asList(expiringBatch);

        when(scanner.nextLine())
                .thenReturn(String.valueOf(daysThreshold))
                .thenReturn(String.valueOf(batchId))
        ...
        String output = outContent.toString();
        assertTrue(output.contains(String.format("Successfully discarded %d units from Batch ID %d.", remainingQuantity, batchId)));
    }
    ...
    void shouldInformNoExpiringBatches() {
        ...
        verify(inventoryManager, times(1)).getAllExpiringBatches(daysThreshold);
        String output = outContent.toString();
        assertTrue(output.contains(String.format("No stock batches found expiring within %d days in the back-store to discard.", daysThreshold)));
    }
    ...
    void shouldHandleInvalidBatchIdNonNumeric() {
        ...
        // Corrected StockBatch constructor usage
        StockBatch expiringBatch = new StockBatch(101, "PROD001", LocalDate.now().minusMonths(1), LocalDate.now().plusDays(5), 45);
        List<StockBatch> expiringBatches = Arrays.asList(expiringBatch);

        when(scanner.nextLine())
                .thenReturn(String.valueOf(daysThreshold))
                .thenReturn(invalidBatchId);
    ...
    void shouldCancelDiscardOperation() {
        ...
        when(scanner.nextLine())
                .thenReturn(String.valueOf(daysThreshold))
                .thenReturn("0"); // User enters 0 to cancel
    ...
    void shouldHandleBatchIdNotFoundInList() {
        ...
        String output = outContent.toString();
        assertTrue(output.contains("Batch ID not found or not in the expiring list. Please select an ID from the list."));
    }
    ...
    void shouldHandleIllegalArgumentException() {
        ...
        when(inventoryManager.getAllExpiringBatches(daysThreshold)).thenReturn(expiringBatches);
        doThrow(new IllegalArgumentException(errorMessage)).when(inventoryManager).discardBatchQuantity(batchId, quantityToDiscard);
    ...
    void shouldHandleGenericException() {
        ...
        when(inventoryManager.getAllExpiringBatches(daysThreshold)).thenReturn(expiringBatches);
        doThrow(new RuntimeException(errorMessage)).when(inventoryManager).discardBatchQuantity(batchId, quantityToDiscard);
    ...
    void shouldDisplayMultipleExpiringBatchesCorrectly() {
        ...
        StockBatch batch1 = new StockBatch(201, "PROD005", LocalDate.now().minusMonths(3), LocalDate.now().plusDays(10), 100);
        StockBatch batch2 = new StockBatch(202, "PROD006", LocalDate.now().minusMonths(1), LocalDate.now().plusDays(5), 75);
        List<StockBatch> expiringBatches = Arrays.asList(batch1, batch2);
        int quantityToDiscard = 10;

        when(scanner.nextLine())
                .thenReturn(String.valueOf(daysThreshold)) // Threshold input
                .thenReturn(String.valueOf(batch1.getId())) // Select batch1
                .thenReturn(String.valueOf(quantityToDiscard)); // Discard quantity

        when(inventoryManager.getAllExpiringBatches(daysThreshold)).thenReturn(expiringBatches);
        doNothing().when(inventoryManager).discardBatchQuantity(batch1.getId(), quantityToDiscard); // Mock discard for batch1
        ...
        assertTrue(output.contains(
                String.format("%-10s %-15s %-15s %-15s %-15s", "Batch ID", "Product Code", "Expiry Date", "Purch Date", "Remaining Qty")
        ));
        // Verify both batches are listed
        assertTrue(output.contains(
                String.format("%-10d %-15s %-15s %-15s %-15d",
                        batch1.getId(), batch1.getProductCode(), batch1.getExpiryDate().toString(), batch1.getPurchaseDate().toString(), batch1.getQuantityRemaining())
        ));
        assertTrue(output.contains(
                String.format("%-10d %-15s %-15s %-15s %-15d",
                        batch2.getId(), batch2.getProductCode(), batch2.getExpiryDate().toString(), batch2.getPurchaseDate().toString(), batch2.getQuantityRemaining())
        ));
        // Verify the successful discard message for the selected batch
        assertTrue(output.contains(String.format("Successfully discarded %d units from Batch ID %d.", quantityToDiscard, batch1.getId())));
    ...
    void shouldHandleBatchWithZeroRemainingQuantity() {
        ...
        StockBatch expiringBatch = new StockBatch(batchId, "PROD007", LocalDate.now().minusMonths(6), LocalDate.now().plusDays(1), initialQuantity);
        ...
        assertTrue(output.contains(
                String.format("%-10d %-15s %-15s %-15s %-15d",
                        expiringBatch.getId(), expiringBatch.getProductCode(), expiringBatch.getExpiryDate().toString(), expiringBatch.getPurchaseDate().toString(), expiringBatch.getQuantityRemaining())
        ));
        assertTrue(output.contains(String.format("Current Remaining Quantity: %d", initialQuantity)));
        assertTrue(output.contains(String.format("Cannot discard %d units. Only %d remaining in batch %d.", 1, initialQuantity, batchId)));
